...
local action = function(msg, blocks)
	...
	if blocks[1] == 'botadded' then
		...
		if db:hget('bot:general', 'adminmode') == 'on' and not roles.is_bot_owner(msg.adder.id) then
			api.sendMessage(msg.chat.id, 'Me las piro vampiro, solo mi dueño @xxdamage puede meterme a grupos. *AUTODEP*', true)
			api.leaveChat(msg.chat.id)
    else
    api.sendMessage(msg.chat.id, 'Buenasssssss, todo bien? Soy Banhammer 4.0 el bot de @xxdamage que a todo el mundo le gusta toquetear :( Dame admin y que empiece la fiesta! Con /help puedes ver mis cualidades y con /comandos , te muestro los comandos basicos de admin :P. Pon por el grupo /lang y selecciona tu idioma. Haz lo mismo por privado', true)
			return
		end
