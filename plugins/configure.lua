...
local function action(msg, blocks)
    ...
    if msg.cb then
        ...
        api.editMessageText(msg.chat.id, msg.message_id, '_Navega por los menus para cambiar los ajustes_', keyboard, true)
    else
        if not roles.is_admin_cached(msg) then return end
        local res = api.sendKeyboard(msg.from.id, '_Navega por los menus para cambiar los ajustes_', keyboard, true)
        if not misc.is_silentmode_on(msg.chat.id) then --send the responde in the group only if the silent mode is off
            if res then
                api.sendMessage(msg.chat.id, '`Te he enviado los menus de configuración por privado`', true)
            else
