...
download_file = function(url, filename)
	...
	local file_path = 'archivos/'..filename
...
function roles.is_bot_admin(chat_id)
...
function misc.sendMessageToAdmins(chat_id, message_text)
	local res, code = api.getChatAdministrators(chat_id)
	if not res then
		return false, code
	end
	for _, admin in pairs(res.result) do
		api.sendMessage(admin.user.id, message_text, true)
	end
end

function misc.forwardToAdmins(chat_id, message_id)
	local res, code = api.getChatAdministrators(chat_id)
	if not res then
		return false, code
	end
	for _, admin in pairs(res.result) do
		api.forwardMessage(admin.user.id, chat_id, message_id)
	end
end
